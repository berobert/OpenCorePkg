...
\emph{Note 2}: Resetting NVRAM will normally also erase any boot options not specified in
\texttt{BlessOverride}, for example Linux installations to custom locations and without the
\texttt{OpenLinuxBoot} driver, or user-specified UEFI boot menu entries. To obtain reset NVRAM
functionality which does not remove boot options, it is possible to use the \texttt{-{}-preserve-boot}
option (though see the warning specified).
