  ...
  )
{
  ...
  for (Index = 0; Index < EntryProtocolHandleCount; ++Index) {
    ...
    if (!Action (PickerContext, BootEntryProtocol, Context)) {
      break;
    }
  }
}
  ...
  )
{
  ...
  //
  // Non-filesystem entries cannot be saved as default picker option.
  //
  if (CreateDefault && (FileSystem->Handle == OC_CUSTOM_FS_HANDLE)) {
